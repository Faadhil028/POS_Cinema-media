                        ...
                        @forelse ($timetables as $timetable)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                                ...
                                <th scope="row"
                                    ...
                                    {{ $timetable->film->title }}</th>
                                <th class="px-6 py-4">
                                    {{ $timetable->studio->name }} ({{ $timetable->studio->class }})
                                </th>
                                <th class="px-6 py-4">
                                    {{ \Carbon\Carbon::parse($timetable->date)->locale('id')->isoFormat('dddd, D MMMM YYYY') }}
                                </th>
                                <th class="px-6 py-4">
                                    {{ \Carbon\Carbon::parse($timetable->start_time)->locale('id')->format('H:i:s') }}
                                </th>
                                <td>
                                    <div class="flex">
                                        <a href="{{ route('admin.timetables.edit', ['timetable' => $timetable->id]) }}"
                                            class="font-medium text-sky-500 hover:underline px-1">Edit</a>
                                        @if ($timetable->transaction->isEmpty())
                                            /
                                            <form method="POST"
